...
- **Target versioning columns (CR-004)**: `targets` menggunakan `valid_from` + `valid_to` + `is_active`, konsisten dengan pola versioning `indicator_versions`. Revisi target: baris lama `is_active=false` + `valid_to` diisi timestamp revisi (field lain tidak berubah); baris baru `revision_no+1`, `is_active=true`, `valid_from` diisi timestamp revisi, `valid_to=NULL`. Lihat §15 log CR-004 untuk detail.
...
> **Phase 8 — Indicator Management** — 🟢 SELESAI (2026-09-11) — Model `Indicator` (identitas, dapat diupdate/dihapus langsung jika belum punya versi) dan `IndicatorVersion` (konfigurasi immutable, versi baru otomatis menonaktifkan versi aktif lama), permission `indicator.manage`/`indicator.view` mengikuti pola `structure.manage`/`.view`, 15 Feature Test baru (70 total lulus). Kategori indikator (`indicator_categories`) ditunda, di luar scope.
> **Phase 9 — Target Management** — 🟢 SELESAI (2026-09-15) — Model `Target` (identitas+konfigurasi tergabung, versioning via `valid_from`/`valid_to`/`is_active` sesuai CR-004), `TargetService` (createInitial/createRevision/delete, guard "satu target aktif" per `indicator_version_id`+`period_label` di Service layer), endpoint CRUD tanpa PUT/PATCH (target immutable), 21 Feature Test baru (91 total lulus). Audit logging domain Target tetap dieskalasi sebagai cross-domain Design Gap, di luar scope Phase 9.
...
| 2026-09-11 | **Phase 8 — Indicator Management selesai.** Model `Indicator` (identitas) dan `IndicatorVersion` (konfigurasi ter-versioning) diimplementasikan. Strategi: `indicators` dapat di-UPDATE langsung dan di-DELETE fisik hanya jika belum punya versi; `indicator_versions` immutable — perubahan konfigurasi selalu berupa INSERT versi baru yang otomatis menonaktifkan versi aktif lama (`is_active=false`, `valid_to` diisi). Permission baru `indicator.manage`/`indicator.view` mengikuti pola assignment `structure.manage`/`structure.view` persis. Kategori indikator (`indicator_categories`/`indicator_category_assignments`) sengaja ditunda, di luar scope Phase 8 (konsisten pola `MeasurementDirection` di Phase 6). | Konsisten arsitektur versioning-over-overwrite §3.1/§10; keputusan role assignment & scope disetujui eksplisit pemilik proyek |
| 2026-09-15 | **Architecture Decision (CR-004) — Tambah kolom `valid_from` dan `valid_to` pada tabel `targets`**, menyelaraskan mekanisme versioning `targets` dengan pola yang sudah dipakai `indicator_versions` (`valid_from`+`valid_to`+`is_active`). Migration bersifat additive (2 kolom nullable timestamp), tidak mengubah kolom/FK/index existing. 0 baris data existing pada `targets` saat migration dijalankan — tidak ada backfill/rekonstruksi timestamp historis yang diperlukan. Siklus migrate→rollback→re-migrate diverifikasi lulus penuh tanpa error, schema kembali konsisten di setiap tahap. Bagian dari persiapan Phase 9 — Target Management (implementasi domain Target itu sendiri belum dimulai). | Menutup inkonsistensi antara wording `CLAUDE.md §10` (yang sudah menyebut `valid_to` untuk revisi target) dengan schema `targets` aktual yang sebelumnya hanya memiliki `is_active`; menghindari dua pola versioning berbeda dalam satu proyek; disetujui eksplisit pemilik proyek melalui proses Change Management §17 |
| 2026-09-15 | **Phase 9 — Target Management selesai.** Model `Target` (identitas+konfigurasi dalam satu baris, tidak dipisah seperti `Indicator`/`IndicatorVersion`) diimplementasikan. Strategi: revisi target = INSERT baris baru (`revision_no+1`, `is_active=true`, `valid_from=now()`) + baris lama `is_active=false`+`valid_to=now()` (field lain tidak berubah), dalam satu DB transaction — sama sekali tidak ada `UPDATE` yang mengubah data historis substantif. Guard "satu target aktif per `indicator_version_id`+`period_label`" ditegakkan di Service layer (tanpa DB unique constraint), konsisten pola `IndicatorVersion` Phase 8; race condition pada guard ini diterima sebagai known limitation (bukan gap baru). `reason` wajib saat revisi (validasi FormRequest + Service safety net), boleh NULL untuk target awal — sesuai pemisahan data-integrity (DB tetap nullable) vs business-rule (aplikasi). Delete fisik hanya diizinkan jika target belum punya `realizations`, dicek via Query Builder langsung (bukan relasi Eloquent, karena Model `Realization` belum ada — scope Phase 10). Endpoint: `GET/POST /targets`, `GET /targets/{id}`, `GET /targets/{id}/revisions`, `POST /targets/{id}/revisions`, `DELETE /targets/{id}` — tanpa `PUT`/`PATCH` (target immutable). Permission `target.manage` (Admin, sesuai AD-2) dan `target.view` (Super Admin/Kepala Dinas/Pimpinan) dipakai; `target.discuss` tetap ada di RBAC namun tidak dikonsumsi endpoint manapun (pembahasan target tetap proses organisasi/offline sesuai AD-2); `target.approve-revision` tetap tidak di-assign (TBD-3 CR-001, tidak diselesaikan). 21 Feature Test baru (`TargetTest.php`), regresi penuh 91 test/172 assertions lulus, 0 gagal. Audit logging untuk domain Target (§11 CLAUDE.md) **tidak diimplementasikan** pada Phase 9 — INSPECT menemukan tidak ada infrastruktur audit (Model/Service/Trait/Observer) sama sekali di codebase meski tabel `audit_logs` sudah ada sejak Phase 3; membangun ini khusus untuk Target akan menciptakan precedent audit parsial (Target teraudit, Indikator/Struktur/status Realisasi/hak akses tidak), sehingga dieskalasi sebagai **Cross-Domain Design Gap** tersendiri, di luar scope Phase 9. | Konsisten arsitektur versioning-over-overwrite §3.1/§10 dan pola Service Layer tanpa Repository; seluruh keputusan desain (versioning columns, endpoint terpisah untuk revisi, strategi delete-guard, penundaan audit infrastructure) melalui proses INSPECT→ANALYZE→DECISION→DESIGN→IMPLEMENT dengan persetujuan eksplisit pemilik proyek di setiap checkpoint |
