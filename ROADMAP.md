...
## Phase 9 — Target Management — 🟢 COMPLETED
...
- **Prerequisite**: Phase 8 selesai. Open Question CR-001 #3 (kewajiban approval Kadis atas revisi target) **tetap terbuka** (TBD-3) — tidak menjadi blocker penyelesaian Phase 9; `target.approve-revision` sengaja tidak di-assign ke role manapun, konsisten baseline Phase 5.
- **Pekerjaan tambahan (CR-001)**: referensi dokumen perencanaan pada proses penetapan target dipenuhi via `planning_document_id` (kolom sudah ada sejak Phase 3/CR-002, NOT NULL di `targets`, wajib diisi setiap penetapan/revisi — additive, tidak ada perubahan skema baru untuk kebutuhan ini di Phase 9). Permission check: hanya `admin` (`target.manage`) yang bisa eksekusi CRUD target; `kepala_bidang`/`sekretaris` hanya punya `target.discuss` (tidak terhubung ke endpoint eksekusi apa pun — pembahasan tetap proses organisasi/offline sesuai AD-2, bukan endpoint sistem).
...
- **Status**: 🟢 **Completed** — Model `Target` (identitas+konfigurasi dalam satu baris, versioning via `valid_from`/`valid_to`/`is_active`, kolom ditambah lewat CR-004 karena schema awal Phase 3 hanya punya `is_active`). Revisi target = INSERT baris baru (`revision_no+1`) + baris lama dinonaktifkan (`is_active=false`, `valid_to` diisi) dalam satu DB transaction — tidak pernah `UPDATE` yang menghapus nilai historis, memenuhi AC "versi lama tetap terhubung ke realisasi lama" (`realizations.target_id` tidak pernah berpindah). `reason` wajib divalidasi (FormRequest + Service) khusus saat revisi (`revision_no > 1`), memenuhi AC "revisi target wajib alasan". Permission `target.manage` di-enforce di level API (middleware, bukan hanya UI) — diverifikasi eksplisit lewat Feature Test (operator/kepala_bidang/sekretaris ditolak 403 pada endpoint mutasi). Delete fisik hanya diizinkan jika target belum punya realisasi. 21 Feature Test baru, regresi penuh 91 test/172 assertions lulus. Audit logging domain Target (§11 CLAUDE.md) **tidak diimplementasikan** — dieskalasi sebagai Cross-Domain Design Gap (infrastruktur audit belum ada sama sekali di codebase, bukan spesifik Phase 9). Selesai pada 2026-09-15.
